/**
 * pipelines.ts — Pipeline API 函数
 *
 * 类型定义已拆分到 pipelines.types.ts
 * v125修复：统一 client→apiClient 引用；ApiResponse 从 client 导入；
 *           PagesWrapper/EvalRoundsWrapper/OperatorsWrapper 定义在本文件
 */
...
export type {
  ...
  EvaluatorStepData,
  MetaStepData,
  TranslatorRoundRecord,
  TranslatorStepData,
  ScannerStepData,
  DbCheckStepData,
  GeneratedPageFull,
  UpdatePageDecisionRequest,
  VerifyStepData,
  RestartFromStepRequest,
...
function extractData<T>(resp: { data: { data?: T } }): T { return resp.data.data as T }

// ==================== API方法 ====================

export async function getPipelines(): Promise<PipelineListResponse> {
  const res = await apiClient.get<ApiResponse<PipelineListResponse>>('/pipelines')
...
export async function getPipelineDetail(id: string): Promise<PipelineDetailResponse> {
  const res = await apiClient.get<ApiResponse<PipelineDetailResponse>>('/pipelines/' + id)
...
export async function createPipeline(req: CreatePipelineRequest): Promise<PipelineDetailResponse> {
  const res = await apiClient.post<ApiResponse<PipelineDetailResponse>>('/pipelines', req)
  return extractData<PipelineDetailResponse>(res)
}

export async function startPipeline(id: string): Promise<PipelineDetailResponse> {
  const res = await apiClient.post<ApiResponse<PipelineDetailResponse>>('/pipelines/' + id + '/start', null, {
    timeout: 3600000,
  })
  return extractData<PipelineDetailResponse>(res)
}
...
export async function cancelPipeline(id: string): Promise<PipelineDetailResponse> {
  const res = await apiClient.post<ApiResponse<PipelineDetailResponse>>('/pipelines/' + id + '/cancel')
  return extractData<PipelineDetailResponse>(res)
}

export async function deletePipeline(id: string): Promise<void> {
  await apiClient.delete<ApiResponse<void>>('/pipelines/' + id)
}

export async function getStepDetail(pipelineId: string, stepName: string): Promise<StepDetailResponse> {
  const res = await apiClient.get<ApiResponse<StepDetailResponse>>('/pipelines/' + pipelineId + '/steps/' + stepName)
...
export async function restartFromStep(
  ...
  const res = await apiClient.post<ApiResponse<PipelineDetailResponse>>(
...
export async function getEvalRounds(pipelineId: string): Promise<EvalRoundDetail[]> {
  const res = await apiClient.get<ApiResponse<EvalRoundsWrapper>>('/pipelines/' + pipelineId + '/eval-rounds')
...
export async function getGeneratedPages(pipelineId: string): Promise<GeneratedPageFull[]> {
  const res = await apiClient.get<ApiResponse<PagesWrapper>>('/pipelines/' + pipelineId + '/pages')
...
export async function finalizePipeline(pipelineId: string): Promise<void> {
  await apiClient.post<ApiResponse<void>>('/pipelines/' + pipelineId + '/finalize')
}

// ==================== P7新增：二级审批API ====================

export async function submitFinalize(pipelineId: string): Promise<void> {
  await apiClient.post<ApiResponse<void>>('/pipelines/' + pipelineId + '/submit-finalize')
}

export async function confirmFinalize(pipelineId: string): Promise<void> {
  await apiClient.post<ApiResponse<void>>('/pipelines/' + pipelineId + '/confirm-finalize')
}
...
export async function markPassed(pipelineId: string): Promise<void> {
  await apiClient.post<ApiResponse<void>>('/pipelines/' + pipelineId + '/mark-passed')
}
...
export async function aiFixPage(
  ...
  const res = await apiClient.post<ApiResponse<AIFixPageResponse>>(
...
export async function rollbackPageHTML(
  ...
  const res = await apiClient.post<ApiResponse<RollbackPageResponse>>(
...
export async function verifyPipeline(pipelineId: string): Promise<PipelineDetailResponse> {
  const res = await apiClient.post<ApiResponse<PipelineDetailResponse>>('/pipelines/' + pipelineId + '/verify', null, {
    timeout: 1800000,
  })
  return extractData<PipelineDetailResponse>(res)
}
...
export async function batchCreatePipelines(courseCodes: string[]): Promise<BatchCreateResult> {
  const res = await apiClient.post<ApiResponse<BatchCreateResult>>('/pipelines/batch-create', {
...
export async function batchStartPipelines(pipelineIds: string[]): Promise<BatchStartResult> {
  const res = await apiClient.post<ApiResponse<BatchStartResult>>('/pipelines/batch-start', {
...
export async function getOperators(): Promise<OperatorInfo[]> {
  const res = await apiClient.get<ApiResponse<OperatorsWrapper>>('/pipelines/operators')
...
export async function batchAssignPipelines(
  ...
  const res = await apiClient.post<ApiResponse<BatchAssignResult>>('/pipelines/batch-assign', {
    pipeline_ids: pipelineIds,
...
export async function batchRestartFromStep(
  ...
  const res = await apiClient.post<ApiResponse<BatchRestartResult>>(
...
export async function publishPipeline(id: string): Promise<void> {
  await apiClient.post(`/pipelines/${id}/publish`)
}
...
export async function getPipelineAvailableRounds(id: string): Promise<number[]> {
  const res = await apiClient.get<{ code: number; data: { available_rounds: number[] } }>(`/pipelines/${id}/history`)
...
export async function getPipelineHistory(id: string, round: number): Promise<PipelineHistoryResponse> {
  const res = await apiClient.get<{ code: number; data: PipelineHistoryResponse }>(`/pipelines/${id}/history?round=${round}`)
...
export async function getPagesMeta(pipelineId: string): Promise<GeneratedPageFull[]> {
  const res = await apiClient.get<ApiResponse<PagesWrapper>>('/pipelines/' + pipelineId + '/pages-meta')
  const data = extractData<PagesWrapper>(res)
  return data.pages
}
...
export async function getSinglePageHTML(pipelineId: string, pageNumber: number): Promise<GeneratedPageFull> {
  const res = await apiClient.get<ApiResponse<GeneratedPageFull>>('/pipelines/' + pipelineId + '/pages/' + pageNumber + '/html')
