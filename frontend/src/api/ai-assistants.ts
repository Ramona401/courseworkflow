...
export type AssistantScene =
  | "review_workbench"
  | "courseware_review" // 独立全屏评审工作台
  | "courseware_self_review" // 课件作者提交前自审
...
export const ASSISTANT_SCENE_LABELS: Record<AssistantScene, string> = {
  review_workbench: "教案评审工作台",
  courseware_review: "课件审核",
  courseware_self_review: "课件作者自审",
...
export function isValidSharePolicy(
  p: string | null | undefined,
): p is AssistantSharePolicy {
...
export async function listAssistants(
  ...
  if (params?.lesson_plan_id) {
    query.set("lesson_plan_id", params.lesson_plan_id);
  }
  if (params?.only_active !== undefined)
    query.set("only_active", String(params.only_active));
  const qs = query.toString();
  const url = `/ai-assistants${qs ? "?" + qs : ""}`;
  const res = await client.get<{ code: number; data: AIAssistantListResponse }>(
    url,
  );
...
export async function getMyPublishGroups(): Promise<MyPublishGroupsResponse> {
  const res = await client.get<{ code: number; data: MyPublishGroupsResponse }>(
    "/ai-assistants/my-groups",
  );
  return res.data.data!;
}
...
export async function getAssistant(id: string): Promise<AIAssistant> {
  const res = await client.get<{ code: number; data: AIAssistant }>(
    `/ai-assistants/${id}`,
  );
  return res.data.data!;
}
...
export async function createAssistant(
  req: CreateAIAssistantRequest,
): Promise<AIAssistant> {
  const res = await client.post<{ code: number; data: AIAssistant }>(
    "/ai-assistants",
    req,
  );
  return res.data.data!;
 ...
 */
export function parseDefaultScenes(
  defaultScenesJSON: string,
): AssistantScene[] {
 ...
 *   }
 */
export function groupAssistantsBySource(
  items: AIAssistantListItem[],
): Record<AssistantSource, AIAssistantListItem[]> {
            ...
            switch (eventType) {
              ...
              case "components":
                if (Array.isArray(data.components)) {
                  handlers.onComponents?.(
                    data.components as DesignerComponentBrief[],
                  );
                }
                break;
              case "chunk":
              ...
              case "draft_update":
                if (typeof data.draft === "string")
                  handlers.onDraftUpdate?.(data.draft);
                break;
              case "done":
                handlers.onDone?.(
                  typeof data.reply === "string" ? data.reply : "",
                  typeof data.draft === "string" ? data.draft : "",
                  Array.isArray(data.referenced)
                    ? (data.referenced as string[])
                    : [],
                );
                isClosed = true;
                break;
              case "error":
                handlers.onError?.(
                  typeof data.error === "string" ? data.error : "AI 调用失败",
                );
                isClosed = true;
                break;
            }
