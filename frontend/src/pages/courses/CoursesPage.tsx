...
import {
  getCourses, getOSSCatalog, fetchCourseIndex,
  type CourseListItem, type OSSModuleItem,
} from '@/api/courses'
import client from '@/api/client'
import {
  BookOpen, Download, RefreshCw, Plus, Search,
  CheckCircle, AlertCircle, Layers, Zap, Square, CheckSquare,
} from 'lucide-react'
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        <div style={stat}><div style={{ fontSize: 11, color: '#8e8e93', fontWeight: 600, marginBottom: 4 }}>已注册课程</div><div style={{ fontSize: 28, fontWeight: 700, color: '#1c1c1e' }}>{totalCourses}</div></div>
        <div style={stat}><div style={{ fontSize: 11, color: '#8e8e93', fontWeight: 600, marginBottom: 4 }}>已有索引</div><div style={{ fontSize: 28, fontWeight: 700, color: '#34c759' }}>{withIndex}</div></div>
        <div style={stat}><div style={{ fontSize: 11, color: '#8e8e93', fontWeight: 600, marginBottom: 4 }}>待拉取索引</div><div style={{ fontSize: 28, fontWeight: 700, color: '#ff9500' }}>{withoutIndex}</div></div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          {isAdmin && courses.length > 0 && <button style={btnO} onClick={handleBatchFetch} disabled={batchRunning}><Download size={14} /> {batchRunning ? '处理中...' : '批量拉取索引'}</button>}
      ...
      <div style={card}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 40 }}>
            ...
            {isAdmin && <div style={{ color: '#007aff', fontSize: 13, marginTop: 8, cursor: 'pointer' }} onClick={openCatalog}>点击从OSS目录注册 →</div>}
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead><tr style={{ borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
              {['课程编号','课程名称','模块ID','索引状态','页面数'].map(h => (
                <th key={h} style={{ textAlign: h === '课程编号' || h === '课程名称' ? 'left' : 'center', padding: '10px 12px', color: '#8e8e93', fontWeight: 600, fontSize: 11 }}>{h}</th>
              ))}
              {isAdmin && <th style={{ textAlign: 'center', padding: '10px 12px', color: '#8e8e93', fontWeight: 600, fontSize: 11 }}>操作</th>}
            </tr></thead>
            <tbody>{courses.map(c => (
              <tr key={c.id} style={{ borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                <td style={{ padding: 12, fontWeight: 600, color: '#007aff', fontFamily: 'monospace' }}>{c.course_code}</td>
                <td style={{ padding: 12, color: '#1c1c1e' }}>{c.course_name}</td>
                <td style={{ padding: 12, textAlign: 'center', color: '#8e8e93', fontFamily: 'monospace', fontSize: 12 }}>{c.external_module_id || '-'}</td>
                <td style={{ padding: 12, textAlign: 'center' }}>
                  {c.has_index ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: '#34c759', fontSize: 12 }}><CheckCircle size={14} /> 已就绪</span>
                ...
                <td style={{ padding: 12, textAlign: 'center', fontFamily: 'monospace', fontSize: 12, color: c.has_index ? '#1c1c1e' : '#c7c7cc' }}>
                  {c.has_index ? c.index_page_count + '页 / ' + (c.index_total_length > 1000 ? (c.index_total_length / 1000).toFixed(1) + 'K' : c.index_total_length + '字') : '-'}
                </td>
                {isAdmin && <td style={{ padding: 12, textAlign: 'center' }}>
                  <button style={{ ...btn, fontSize: 12, padding: '5px 12px', color: '#007aff', borderColor: 'rgba(0,122,255,0.2)' }}
                    onClick={() => handleFetchIndex(c.course_code)} disabled={fetchingIndex === c.course_code}>
                    {fetchingIndex === c.course_code ? <><RefreshCw size={12} style={{ animation: 'spin 1s linear infinite' }} /> 拉取中...</> : <><Download size={12} /> {c.has_index ? '重新拉取' : '拉取索引'}</>}
                  </button>
                </td>}
              </tr>
            ))}</tbody>
            ...
            <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 12, alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#f5f5f7', borderRadius: 10, padding: '8px 12px', flex: 1 }}>
                  ...
                  <input type="text" placeholder="搜索课程编号或名称（如 G1-02）..."
                ...
                {!catalogLoading && filteredSelectable.length > 0 && (
                  <button style={{ ...btn, fontSize: 12, padding: '7px 12px', color: '#007aff' }} onClick={toggleSelectAll}>
                    {filteredSelectable.every(m => selectedIds.has(m.id)) ? <><CheckSquare size={13} /> 取消全选</> : <><Square size={13} /> 全选可注册</>}
                  </button>
                )}
              </div>
              ...
              {selectedCount > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 10, padding: '10px 14px', background: 'rgba(0,122,255,0.06)', borderRadius: 10, border: '1px solid rgba(0,122,255,0.15)' }}>
                  ...
                  <button style={{ ...btnG, fontSize: 12, padding: '6px 16px' }}
                    onClick={handleBatchRegisterSelected} disabled={batchRunning}>
                    {batchRunning ? <><RefreshCw size={12} style={{ animation: 'spin 1s linear infinite' }} /> 注册中...</> : <><Zap size={12} /> 批量注册+拉取</>}
                  </button>
                </div>
              )}
            </div>
                  ...
                  return (
                    ...
                    onClick={() => { if (canRegister) toggleSelect(mod.id) }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: '#1c1c1e', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{(mod.name || '').trim()}</div>
                        <div style={{ fontSize: 11, color: '#8e8e93', marginTop: 2, display: 'flex', gap: 12 }}>
                          <span>ID: {mod.id}</span>
                          <span>{mod.lesson_count} 课时</span>
                          {mod.has_index ? <span style={{ color: '#34c759' }}>有索引</span> : <span style={{ color: '#c7c7cc' }}>无索引</span>}
                        </div>
                      </div>
                      ...
                      <div style={{ flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                        {mod.is_registered ? (
                          ...
                          <button style={{ ...btnP, fontSize: 12, padding: '5px 12px' }}
                            onClick={() => handleRegisterOne(mod)} disabled={registeringId === mod.id || batchRunning}>
                            {registeringId === mod.id ? <><RefreshCw size={12} style={{ animation: 'spin 1s linear infinite' }} /> 注册中...</> : <><Zap size={12} /> 注册+拉取</>}
                          </button>
                        ) : (
