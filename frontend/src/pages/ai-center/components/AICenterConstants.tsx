 ...
 *
 * 本轮修复：
 *   - Google Gemini静态候选补充平台实际使用的Gemini 3.5 Flash；
 *   - ModelSelect监听value和availableModels变化；
 *   - 切换场景、重新查询模型或保存回填后，手工输入态不再残留旧场景状态。
 */
...
export const AI_PROVIDERS: AIProvider[] = [
  {
    ...
    description: '最强推理能力，长上下文，适合复杂任务',
    isChinese: false,
  },
  {
    id: 'openai',
  ...
  },
  {
    ...
    description: '国产顶尖，代码和推理能力极强，性价比高',
    isChinese: true,
  },
  {
    id: 'qwen',
    ...
    description: '阿里云出品，中文理解强，企业服务稳定',
    isChinese: true,
  },
  {
    id: 'doubao',
    ...
    description: '字节跳动出品，中文对话流畅，价格亲民',
    isChinese: true,
  },
  {
    id: 'moonshot',
    ...
    description: '超长上下文专家，128k窗口，适合长文档处理',
    isChinese: true,
  },
  {
    id: 'zhipu',
  ...
  },
  {
    ...
    description: '百度出品，知识图谱丰富，中文搜索增强',
    isChinese: true,
  },
  {
    id: 'spark',
    ...
    description: '科大讯飞出品，语音+语言一体化，教育场景优化',
    isChinese: true,
  },
  {
    id: 'oneapi',
    ...
    description: '自建中转站（One API/New API），统一管理多个供应商',
    isChinese: false,
  },
  {
    id: 'ollama',
    ...
    description: '本地离线运行，数据不出境，适合内网部署',
    isChinese: false,
  },
]
...
export function ModelSelect({
  ...
  const [showCustomInput, setShowCustomInput] =
    useState(valueIsCustom)

  const [customValue, setCustomValue] =
    useState(valueIsCustom ? value : '')

  // 场景切换、保存回填、模型列表重新查询时，
  // 必须以父组件传入的真实值重新同步内部编辑状态。
  useEffect(() => {
    const custom =
      value !== null &&
      value !== '' &&
    ...
    >
      ...
      >
        ...
        {availableModels.length > 0 && (
          <optgroup
            label={`可用模型（${availableModels.length}个）`}
          >
