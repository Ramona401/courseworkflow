...
import PagePreviewBlock, {
  MsgBar,
} from "./components/courseware-workshop/PagePreviewBlock";
...
import DeliveryModeSelect, {
  type DeliveryMode,
} from "./components/courseware-workshop/DeliveryModeSelect";
import AutoAssemblyPanel from "./components/courseware-workshop/AutoAssemblyPanel";
import CWSelfReviewPanel from "./components/courseware-workshop/CWSelfReviewPanel";
import CWOwnerReviewRemediationPanel from "./components/courseware-workshop/CWOwnerReviewRemediationPanel";
import CoursewareAssistantPanel from "./components/courseware-workshop/CoursewareAssistantPanel";
import {
  COURSEWARE_WORKSHOP_TABS,
  isCoursewareWorkshopTabVisible,
  type CoursewareWorkshopTab,
} from "./components/courseware-workshop/coursewareWorkshopTabs";
import {
  resolveCoursewareAssistantSelectedPage,
} from "./components/courseware-workshop/coursewareAssistantSelection";
...
const LazySubjectToolsPanel = lazy(
  () => import("./components/courseware-workshop/SubjectToolsPanel"),
);

const LazyCoursewareComicWorkshopPanel = lazy(
  () =>
    import(
      "./components/courseware-workshop/CoursewareComicWorkshopPanel"
    ),
);
...
export default function CoursewareWorkshopPage() {
  ...
  const activeCollab =
    courseware?.collab_state === "in_session" &&
    (courseware?.collab_member_count ?? 0) > 0;
  // 参与者视图（非作者非admin）只在'真集体备课'时才成立：这样幽灵会话不会把普通访客误标为参与者、
  //   也不会把 Step5 工作台错误锁定给他。真会话下参与者体验完全不变。
  const isParticipant = !!(
    user &&
    courseware &&
    user.id !== courseware.user_id &&
    user.role !== "admin" &&
    activeCollab
  );
  // AI自审是作者提交前的个人质量检查，只向课件作者本人开放。
  const isOwner = !!(user && courseware && user.id === courseware.user_id);
  ...
  const [wsTab, setWsTab] =
    useState<CoursewareWorkshopTab>(
      "refine",
    ); // W3: 背景/字体独立Tab；作者专属AI自审加入最终确认页

  /**
   * 整改项向页面微调传递的一次性确认指令。
   *
   * 父组件先通过稳定page_id确定当前页面，
   * 再切换到微调Tab；RefinePanel消费后立即清除。
   */
  const [
    refineInstructionInjection,
    setRefineInstructionInjection,
  ] = useState<{
    id: string;
    coursewareId: string;
    reviewItemId: string;
  ...
  const reloadAnnotations = useCallback(async () => {
    if (!id) return;
    try {
      const r = await listCWAnnotations(id);
      setCwAnnotations(r.annotations || []);
    } catch {
      /* 静默:批注非核心流程,失败不阻断工坊 */
    }
  }, [id]);
  ...
  const handleSetAnchor = async (
    ...
    // W1: notify把提示路由回MediaManagerPanel消息条
    ...
    try {
      ...
      notify?.(
        "✅ 已设为风格锚点！后续生成的配图将自动参考此图风格，保持全课件视觉统一",
      );
    } catch (e) {
      notify?.(
        "❌ 设置锚点失败: " + (e instanceof Error ? e.message : "未知错误"),
      );
    } finally {
      setAnchorSetting("");
    }
  ...
  const handleClearAnchor = async (notify?: (msg: string) => void) => {
    // W1: notify同上, 顶部锚点条调用时不传(静默)
    if (!id || anchorClearing) return;
    if (!confirm("确定清除风格锚点？清除后新生成的配图将不再自动套用此风格。"))
      return;
    ...
    try {
      ...
      notify?.(
        "❌ 清除锚点失败: " + (e instanceof Error ? e.message : "未知错误"),
      );
    } finally {
  ...
  const loadCourseware = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const d = await getCourseware(id);
      setCourseware(d);
      setPages(d.pages || []);
      const hasNav = !!(d.nav_template_html && d.nav_template_html.trim());
      // P0-1: 预览页只检查第1页（封面页）
      const hasPreview = (d.pages || []).some(
        (p) => p.html_content && p.page_number === 1,
      );
      ...
      if (gp.length > 0) {
        ...
        if (buildPreviewNum === 0 && gp.length > 0) {
          ...
          setBuildPreviewNum(
            cwAllDone ? gp[0].page_number : gp[gp.length - 1].page_number,
          );
        }
      }
    } catch {
  ...
  const handleRollback = async (targetStatus: string, targetStep: number) => {
    ...
    try {
      ...
      alert("回退失败: " + (e instanceof Error ? e.message : "未知错误"));
    } finally {
    ...
    try {
      ...
      sseRef.current = subscribeCWIndexSSE(id, {
        ...
        onGenPage: (d) => {
          ...
          setBuildPreviewNum((prev) =>
            d.page_number > prev ? d.page_number : prev,
          );
        },
        ...
        onGenDone: (d) => {
          ...
          if (d.fail_count > 0) {
            setBuildMessage(
              `⚠️ ${d.message}。失败的页面可点下方"继续生成"自动重试补齐（成功页不会重做）。`,
            );
            loadCourseware();
          } else {
        ...
        onError: (d) => {
          setBuildMessage(
            `❌ ${d.message}`,
          ); /* 单页错误不终止整批：不再 setBuildRunning(false)，等 gen_done 统一收尾 */
        },
        ...
        onReconnected: async () => {
          ...
          try {
            ...
            setBuildMessage(
              "🔄 连接已恢复，但同步进度失败，可手动刷新页面查看",
            );
          }
        },
      });
    } catch {
      setBuildMessage("❌ 启动失败");
      setBuildRunning(false);
    }
  };
  useEffect(() => {
    ...
    const t = setInterval(async () => {
      try {
        ...
        if (d.status === "preview") {
          ...
          setGeneratedPages(gp);
          /* v0.42.13: 不再重置预览到第1页，保留当前/最后生成页 */ sseRef.current?.close();
        }
      } catch {}
    }, 15000);
  ...
  const handleCancelBuild = async () => {
    ...
    try {
      ...
      setBuildMessage(
        "⏸ 已发送停止信号，正在结束当前页（已生成页面已保留）...",
      );
    } catch (e) {
      setBuildMessage(
        "❌ 停止失败: " + (e instanceof Error ? e.message : "未知错误"),
      );
    }
  };
  ...
  const openSlideshow = (pn?: number) => {
    ...
    setSlideshowInitPage(
      pn || buildPreviewNum || allPages[0]?.page_number || 1,
    );
  ...
  const handleFullscreen = (pn: number) => {
    setFullscreenPageNum(pn);
    setFullscreenOpen(true);
    setFullscreenCodeView(false);
  };
  ...
  /**
   * 将已确认整改指令注入其稳定页面的微调草稿。
   *
   * page_number_snapshot只用于展示审核时页码；
   * 真正定位必须使用不会随页面重排变化的page_id。
   */
  const handleInjectReviewInstruction =
    useCallback(
      (
        item: CWAIReviewItem,
      ) => {
        ...
        if (!currentPage) {
          alert(
            "原整改页面已删除或当前页面列表尚未同步，请刷新课件后重新检查。",
          );
          return;
        }
  ...
  if (loading)
    return (
      <div
        style={{ textAlign: "center", padding: "80px 0", color: C.textMuted }}
      >
  ...
  const sc = CW_STATUS_CONFIG[courseware.status] || {
    label: courseware.status,
    color: "#6B7280",
    bg: "#F3F4F6",
  };

  const allSlideshowPages =
    generatedPages.length > 0 ? generatedPages : previewPages;

  /**
   * 教学智能体不维护第二套选中页面状态。
   *
   * buildPreviewNum仍是Step 5唯一页面选择真相源，
   * 此处只把页码映射为后端要求的稳定page_id。
   */
  const selectedAssistantPage =
    resolveCoursewareAssistantSelectedPage(
      generatedPages,
      buildPreviewNum,
    );
  ...
  const cwDoneCount = pages.filter(
    (p) => p.html_content && p.html_content.trim(),
  ).length;
  ...
  const batchTotal =
    buildProgress.total > 0 ? buildProgress.total : cwRemainingCount;
  ...
  const batchPercent =
    batchTotal > 0 ? Math.round((batchDone / batchTotal) * 100) : 0;
    ...
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 13, color: C.textMuted, marginTop: 4 }}>
          {courseware.source_type === "topic_direct"
            ? "💡 主题创建"
            : courseware.source_type === "ppt_upload"
              ? "📊 PPT上传"
              : courseware.source_type === "doc_upload"
                ? "📄 文档上传"
                : "📝 " + (courseware.lesson_plan_title || "未知")}{" "}
          &nbsp;|&nbsp; 📚 {courseware.subject} &nbsp;|&nbsp; 🎓{" "}
          {courseware.grade}
        </div>
        ...
        {isParticipant && (
          ...
          >
            👥 你正在参与本课件的<b>集体备课</b>
            ，可在下方工作台微调页面、添加批注。课件的生成、风格、发布等由作者管理。
          </div>
        )}
        ...
        {courseware.style_anchor_asset_id && !isParticipant && (
          ...
          >
            ...
            <span
              ...
              title={courseware.style_anchor_vaoci}
            >
            ...
            <button
              ...
              style={{
                ...
                color: C.danger,
                fontSize: 12,
                cursor: anchorClearing ? "default" : "pointer",
                whiteSpace: "nowrap",
              }}
            >
        ...
        >
          {STEPS.map((s, i) => {
            const active = i === activeStep,
              ...
              reached = i <= maxStepReached;
            return (
              <div
                key={s.key}
                onClick={() => {
                  if (reached && !active) goToStep(i);
                }}
                style={{
                  flex: 1,
                  textAlign: "center",
                  cursor: reached && !active ? "pointer" : "default",
                }}
              >
                <div
                  style={{
        ...
        {activeStep === 4 && (
          <div>
            ...
            {batchTotal > 0 && (
              <div style={{ marginBottom: 20 }}>
                ...
                >
                  ...
                  <span>
                    本次需生成 {batchTotal} 页 · 已完成{" "}
                    <b style={{ color: C.success }}>{batchDone}</b> 页 · 剩余{" "}
                    {batchRemaining} 页
                  </span>
                </div>
                <div
            ...
            <PagePreviewBlock
              ...
              currentNum={buildPreviewNum}
              onSelectPage={setBuildPreviewNum}
              showSlideshow
              onSlideshow={openSlideshow}
              onFullscreen={handleFullscreen}
            />
            ...
            {!buildRunning && cwInterrupted && (
              ...
              >
                ⚠️ 检测到上次生成被中断，已完成{" "}
                <b>
                  {cwDoneCount}/{cwTotalCount}
                </b>{" "}
                页。点击「继续生成」会<b>跳过已生成的页面</b>，仅生成剩余{" "}
                <b>{cwRemainingCount}</b> 页，无需从头再来。
              </div>
            )}
            ...
            {/* 场景C：全部页面已生成完毕 */}
            {!buildRunning && cwRemainingCount === 0 && cwTotalCount > 0 && (
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                <button
                  onClick={() => openSlideshow()}
                  style={{
            ...
            {buildRunning && (
              ...
              >
                ...
                {cwInterrupted
                  ? `续传中：第 1–${cwDoneCount} 页已完成、不会重做，正在生成剩余页…`
                  : "AI正在逐页生成，请耐心等待..."}
        ...
        {activeStep >= 5 && (
          <div>
            ...
            <PagePreviewBlock
              ...
              onSlideshow={openSlideshow}
              onFullscreen={handleFullscreen}
              editable={!buildRunning}
              coursewareId={id}
              onPagesChanged={refreshPagesOnly}
            />
            ...
            {generatedPages.length > 0 && (
              <>
                ...
                {wsTab === "self_review" && isOwner && (
                  <CWSelfReviewPanel
                    coursewareId={id!}
                    coursewareTitle={courseware.title}
                    subject={courseware.subject}
                    grade={courseware.grade}
                    lessonPlanId={courseware.lesson_plan_id}
                    onSelectPage={(pageNumber) => {
                      setBuildPreviewNum(pageNumber);
                    }}
                    onInjectToRefine={
                      handleInjectReviewInstruction
                    }
                  />
                )}
                {wsTab === "remediation" && isOwner && (
                  <CWOwnerReviewRemediationPanel
                    coursewareId={id!}
                    onSelectPage={(pageNumber) => {
                      setBuildPreviewNum(pageNumber);
                    }}
                ...
                {wsTab === "assistant" && isOwner && (
                  <CoursewareAssistantPanel
                    coursewareId={id!}
                    selectedPage={
                      selectedAssistantPage
                    }
                  />
                )}
                {wsTab === "comic" && isOwner && (
                  ...
                  >
                    <LazyCoursewareComicWorkshopPanel
                      coursewareId={id!}
                      courseware={courseware}
                      pageCount={pages.length}
                      onPagesChanged={async (pageNumber) => {
                        await refreshPagesOnly();
                        setBuildPreviewNum(pageNumber);
                      }}
                    />
                  </Suspense>
                ...
                {wsTab === "background" && (
                  <AppearancePanel
                    mode="background"
                    coursewareId={id!}
                    onSwapped={refreshPagesOnly}
                    disabled={buildRunning}
                    pageNum={buildPreviewNum > 0 ? buildPreviewNum : undefined}
                    cwTitle={courseware.title}
                    cwSubject={courseware.subject}
                    cwGrade={courseware.grade}
                  />
                )}
                {wsTab === "font" && (
                  <AppearancePanel
                ...
                {wsTab === "template" && (
                  <TemplateSavePanel coursewareId={id!} />
                )}
                {wsTab === "annotation" && (
                  <AnnotationPanel
                    coursewareId={id!}
                    ...
                    pageNumber={buildPreviewNum}
                    annotations={cwAnnotations}
      ...
      {fullscreenOpen && allSlideshowPages.length > 0 && (
        <CWFullscreenPreview
          ...
          onSlideshow={(pn) => {
            setBuildPreviewNum(pn);
            setFullscreenPageNum(pn);
            setFullscreenOpen(false);
            setSlideshowInitPage(pn);
            setSlideshowOpen(true);
          }}
        />
      )}
      ...
      {slideshowOpen && allSlideshowPages.length > 0 && (
        <SlideshowPlayer
          pages={allSlideshowPages}
          initialPage={slideshowInitPage}
