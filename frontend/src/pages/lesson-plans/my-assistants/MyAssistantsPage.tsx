...
import DrawerAssistantCard, {
  miniBtn,
} from './DrawerAssistantCard'
...
/** 记住老师当前选择的课程和学习层级。 */
const LS_KEY =
  'tedna_my_assistants_subject'

const LS_GRADE_KEY =
  'tedna_my_assistants_grade'
...
export default function MyAssistantsPage() {
  ...
  useEffect(() => {
    ...
    setGrade(
      assistantLevelOptions[0].value,
    )
  }, [
    domain,
    assistantLevelOptions,
    assistantLevelValues,
    grade,
  ])
  ...
  const [related, setRelated] =
    useState<AIAssistantListItem[]>([])

  const [loading, setLoading] =
    useState(false)

  const [listErr, setListErr] =
    useState<string | null>(null)

  const [forkingId, setForkingId] =
    useState<string | null>(null)
  ...
  const [draftToSave, setDraftToSave] =
    useState('')
  ...
  const handleUseStyleProfile =
    useCallback((profileText: string) => {
      const guide =
  ...
  const groupAnalyzable =
    grouped.group.some(
      assistant =>
        assistant.can_view_prompt,
    )

  const groupForkable =
    grouped.group.some(
      assistant => assistant.can_fork,
    )
