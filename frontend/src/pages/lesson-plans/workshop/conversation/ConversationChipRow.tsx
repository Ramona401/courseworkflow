 ...
 *
 * 阶段自动承接期间旧芯片直接隐藏，不保留“变白的旧按钮”。
 * 等待反馈统一由现有 ThinkingIndicator / 流式 AIBubble 表达。
 */
 ...
 */
export default function ConversationChipRow({
  chips,
  onChipClick,
}: ConversationChipRowProps) {
      ...
      {chips.map(chip => (
        <button
          ...
          style={{
            ...
            borderRadius: '18px',
            fontSize: '13px',
            fontWeight: chip.highlight ? 600 : 500,
            ...
            boxShadow: chip.highlight
              ? '0 3px 10px rgba(79,123,232,0.3)'
