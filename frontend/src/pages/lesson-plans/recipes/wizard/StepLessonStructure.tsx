/**
 * StepLessonStructure — 配方向导步骤3：教案结构定义
 *
 * v79 新增：分步向导式配方创建
 *
 * 【配方搭建一页化 · 批次3】教案结构步骤轻化（A 方案）：
 *   - 顶部新增「跟随系统默认 / 自定义结构」两态开关。
 *     开关状态从 ls.length === 0 推导（不新增任何字段，不动 WizardFormData / 提交 payload）：
 *       ls 为空     → 默认模式（跟随系统默认），隐藏整个编辑器，只显示一句安心文案；
 *       ls 非空     → 自定义模式，展开板块编辑器。
 *   - 切到「自定义结构」时若 ls 为空，自动加载默认模板（loadDefaultStructure）；
 *     切回「跟随系统默认」时清空 ls（= 回到系统默认语义，与原"留空=默认"完全一致）。
 *   - 顶部补文案，区分本步骤（教案结构=成品长什么样）与上一步（备课流程=怎么把教案做出来）。
 *
 * 内容（自定义模式下）：
 *   - 板块编辑器：名称、要求、必填标记、排序
...
export default function StepLessonStructure({ formData, updateForm }: StepLessonStructureProps) {
  ...
  // 批次3：两态开关状态从 ls.length 推导——非空即"自定义模式"
  const isCustomMode = ls.length > 0
  ...
  // ---- 批次3：两态开关切换 ----
  // 切到"自定义结构"：若当前为空自动加载默认模板（已有内容则保留不覆盖）
  const switchToCustom = () => {
    if (ls.length === 0) loadDefaultStructure()
  }
  // 切回"跟随系统默认"：清空 ls = 回到系统默认语义
  const switchToDefault = () => {
    if (ls.length > 0) setLS([])
  }
  ...
  return (
    <div style={stepCardStyle}>
      {/* 顶部提示：说明本步骤是"成品教案长什么样"，并与上一步"备课流程"区分 */}
      <div style={{
        ...
        <div style={{ fontSize: '13px', color: C.primary, lineHeight: 1.7 }}>
          📋 这里定义<strong>成品教案长什么样</strong>——它包含哪些板块（如教学目标、教学过程、作业设计等）。
          <br />
          它不是备课的操作步骤——那是上一步「🔧 备课流程」。
          <strong> 不定义时，AI 使用系统默认格式。</strong>
        </div>
      </div>

      {/* 批次3：两态开关 —— 跟随系统默认 / 自定义结构 */}
      <div style={{
        display: 'flex', gap: '4px', padding: '4px',
        background: '#F3F4F6', borderRadius: '10px', marginBottom: '16px',
      }}>
        <button style={segBtn(!isCustomMode)} onClick={switchToDefault}>
          ✅ 跟随系统默认
        </button>
        <button style={segBtn(isCustomMode)} onClick={switchToCustom}>
          ✏️ 自定义结构
        </button>
      </div>

      {/* 默认模式：隐藏编辑器，只显示一句安心文案 */}
      {!isCustomMode && (
        <div style={{
          padding: '32px 24px', textAlign: 'center', color: C.textSec,
          fontSize: '14px', lineHeight: 1.8,
          background: 'rgba(16,185,129,0.04)', borderRadius: '10px',
          border: '1px solid rgba(16,185,129,0.12)',
        }}>
          <div style={{ fontSize: '28px', marginBottom: '8px' }}>✅</div>
          已选择<strong style={{ color: C.success }}>跟随系统默认</strong>格式，
          这一步无需任何设置，可直接进入下一步。
          <br />
          <span style={{ fontSize: '13px', color: C.textMuted }}>
            想自己定制教案的板块格式？点上方「✏️ 自定义结构」。
          </span>
        </div>
      )}
