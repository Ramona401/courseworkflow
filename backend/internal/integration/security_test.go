...
func TestSecurity_SQLInjection_RecipeName(t *testing.T) {
	...
	// 配方是具体教学资源，使用k12教学Actor而不是mixed平台管理员。
...
func TestSecurity_SQLInjection_PromptContent(t *testing.T) {
	...
	token := LoginAsAdmin(t, server.URL)
	seedPromptSlot(t, "prompt_a", "安全测试初始提示词V1")
...
// TestSecurity_UnauthorizedRecipeDelete 非作者删除他人配方 → 被拒绝。
//
// 配方创建要求具体教学教育域，因此由operator创建k12个人配方，
// 再由同域但非作者的viewer尝试删除，锁定“作者本人专属写权限”。
func TestSecurity_UnauthorizedRecipeDelete(t *testing.T) {
	...
	resp, _ := DoDelete(
		t,
		server.URL+"/api/v1/lesson-plans/recipes/"+recipeID,
		viewerToken,
	)
	if resp.StatusCode == http.StatusOK {
		t.Error("非作者viewer不应能删除operator创建的配方")
	}
}
