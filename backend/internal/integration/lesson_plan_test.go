...
func TestLessonPlan_Delete(t *testing.T) {
	...
	// 当前业务采用软删除：记录保留，但deleted_at必须写入。
	...
	detailResp, _ := DoGet(
		t,
		server.URL+"/api/v1/lesson-plans/plans/"+planID,
		token,
	)
	AssertHTTPStatus(t, detailResp, http.StatusNotFound)
}
