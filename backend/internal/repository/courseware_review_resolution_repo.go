...
func normalizeCWReviewDecisionItemIDs(
	input []string,
) []string {
	result :=
		make(
...
func validateCWReviewCarryoverResolutionRecords(
	...
	recordByID :=
		make(
			map[string]*cwReviewCarryoverRecord,
			len(records),
		)
	...
	for _, itemID := range normalizeCWReviewDecisionItemIDs(
		resolvedIDs,
	) {
		record :=
			recordByID[itemID]

		if !isCWReviewCarryoverRecordResolvable(
			record,
		) {
			return ErrCWReviewDecisionCarryoverInvalid
		}
...
func isCWReviewCarryoverRecordResolvable(
	...
	return cwReviewItemContentHash(
		record.CurrentHTML,
	) == strings.TrimSpace(
		record.AppliedPageHash,
	)
	...
	if pageID != "" {
		...
		if err != nil {
			if errors.Is(
				...
				if invalidateErr :=
					invalidateSelfCoursewareReviewItemTx(
						ctx,
						tx,
						sessionID,
						itemID,
