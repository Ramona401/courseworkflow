...
// 通用读取链必须与精确链一致读写school_system，避免同一大纲在不同入口丢失学制。
//
// 上下文16教育域收口：
...
const courseOutlineSelectColumns = `id, scope, scope_target_id, subject, grade, volume, publisher, school_system, title,
...
const courseOutlineQualifiedSelectColumns = `co.id, co.scope, co.scope_target_id, co.subject, co.grade, co.volume, co.publisher, co.school_system, co.title,
