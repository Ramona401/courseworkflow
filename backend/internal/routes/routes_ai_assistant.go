...
//   GET    /api/v1/ai-assistants/my-groups         里程碑一:我能发布到的教研组列表
...
//   POST   /api/v1/ai-assistants/design/chat       P0.5:对话式创作(SSE 流式)
//
...
// 里程碑一改造:新增 /my-groups 固定路径(置于 /{id} 通配兜底之前)
func registerAIAssistantRoutes(
        ...
        //
        // 路径分发优先级(从特殊到通用,固定路径必须排在 /{id} 通配兜底之前):
        //   1. /design/chat      → P0.5 对话式创作(SSE)
        //   2. /my-groups        → 里程碑一:我能发布的教研组列表
        //   3. /{id}/fork        → 复制到"我的"
        //   4. /{id}             → 详情/编辑/删除
        mux.Handle("/api/v1/ai-assistants/",
                middleware.Chain(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
                        ...
                        // v113 P0.5:对话式创作路由(优先匹配,避免被当成 /{id}="design" 处理)
                        ...
                        // 里程碑一:我能发布的教研组列表(固定路径,须早于 /{id} 通配)
