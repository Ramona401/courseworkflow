...
//
// 迭代一 Phase 3.2 改动(本次)：
//   - CreateSchoolUser: 改用 userService.CreateUserWithSchool(req, school.ID, "school_admin_create")，
//     在单一事务内完成"建用户 + 入校(school_members)"，任一步失败整体回滚。
//     删除原"先建用户、再补 AddSchoolMember、失败仅 WARN"的两步降级逻辑——
//     该降级正是孤儿账号(建了用户却不在本校名单)的根源，事务化后从根杜绝。
//
...
// 迭代一 Phase 3.2: 事务化建用户+入校(CreateUserWithSchool)，删除原 WARN 降级
func (h *SchoolAdminHandler) CreateSchoolUser(w http.ResponseWriter, r *http.Request) {
        ...
        // Phase 3.2: 建用户 + 入校 在同一事务内完成，失败整体回滚(不再有 WARN 降级孤儿)
        userInfo, err := h.userService.CreateUserWithSchool(r.Context(), &req, school.ID, "school_admin_create")
        if err != nil {
