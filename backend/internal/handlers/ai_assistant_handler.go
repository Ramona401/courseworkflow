...
//   GET    /api/v1/ai-assistants/my-groups  里程碑一新增:我能发布到的教研组列表
...
//
// share_policy(分享权限策略,本次新增)在 handler 层无需特殊处理:
//   - 请求体里的 share_policy 字段由 json.NewDecoder 自动解进
//     CreateAIAssistantRequest / UpdateAIAssistantRequest(model 已加字段)
//   - 业务拦截全在 service 层(fork 闸门 / edit 收紧 / locked 可见性)
//   - handler 仅需把 service 新抛的两个错误映射成 HTTP 状态码(见 writeServiceError)
...
// ==================== 2. GET /ai-assistants/my-groups 我能发布的教研组 ====================

// MyPublishGroups GET /api/v1/ai-assistants/my-groups
//
// 里程碑一新增:供前端"存为助手"弹窗动态展示货架——
//   - groups:            当前用户担任 lead/backbone 的教研组列表(可发布教研组级助手的目标)
//   - can_publish_group: 是否能发布教研组级助手(groups 非空即 true)
//   - can_publish_school:是否能发布全校级助手(senior_operator / admin 为 true)
//   - can_publish_system:是否能发布系统级助手(仅 admin)
//
// 逻辑零新增,复用 repository.ListMyLeadOrBackboneGroups,只做角色标志位补充
func (h *AIAssistantHandler) MyPublishGroups(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		utils.Fail(w, http.StatusMethodNotAllowed, utils.MsgMethodGetOnly)
		return
	}
	...
	groups, err := repository.ListMyLeadOrBackboneGroups(r.Context(), actor.UserID)
	if err != nil {
		utils.InternalError(w, "查询可发布教研组失败: "+err.Error())
		return
	}
	if groups == nil {
	...
	canPublishSchool := actor.Role == models.RoleSeniorOperator || actor.Role == models.RoleAdmin
	canPublishSystem := actor.Role == models.RoleAdmin

	utils.Success(w, map[string]interface{}{
		"groups":             groups,
		"can_publish_group":  len(groups) > 0,
		"can_publish_school": canPublishSchool,
		"can_publish_system": canPublishSystem,
	})
}

// ==================== 3. GET /ai-assistants/{id} 详情 ====================
...
func (h *AIAssistantHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	a, err := h.service.GetAssistant(r.Context(), actor, id, true) // true:详情请求需原文,无权者由 service 截断保护
	if err != nil {
...
// ==================== 7. POST /ai-assistants/{id}/fork 复制到"我的" ====================
...
//
// share_policy 新增两个错误的映射:
//   - ErrAssistantForkNotAllowed(作者设了"仅可用",不许 fork)→ 403 Forbidden
//   - ErrAssistantInvalidPolicy(share_policy 取值非法)        → 400 BadRequest
func (h *AIAssistantHandler) writeServiceError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, services.ErrAssistantPermDenied),
		errors.Is(err, services.ErrAssistantGroupNotAllowed),
		errors.Is(err, services.ErrAssistantForkNotAllowed):
	...
	case errors.Is(err, services.ErrAssistantNameRequired),
		...
		errors.Is(err, services.ErrAssistantInvalidPolicy),
