...
//   handleReviewError 新增 ErrReviewContentEmpty 分支，归入 400 BadRequest。
//   service 层 ReviewL1/ReviewL2 在 approved 决策且教案正文为空时返回该错误，
//   handler 需将其映射为 400 + 明确文案，审核工作台弹窗的 catch 可直接 alert，
//   提示审核员"教案正文为空，无法审核通过；请退回修改"，而非落入 default 吞成 500。
...
func (h *ReviewV2Handler) handleReviewError(w http.ResponseWriter, err error) {
        ...
        case errors.Is(err, services.ErrReviewNotSubmitted),
                ...
                errors.Is(err, services.ErrReviewContentEmpty),
                errors.Is(err, services.ErrReviewInvalidDecision):
                // v168：ErrReviewContentEmpty 归入 400——"教案正文为空无法通过"是
                // 请求内容不合格，审核工作台弹窗 catch 后可直接展示该文案。
                utils.BadRequest(w, err.Error())
