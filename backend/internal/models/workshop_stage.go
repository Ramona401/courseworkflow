...
// BugFix（自定义阶段提示词无法保存）：
//   - CustomStageResponse 新增 SystemPrompt / PromptVariants / OutputFormat 三个全文字段。
//     根因：原响应只回 has_prompt 布尔值，前端编辑自定义阶段时无法回填提示词原文，
//     导致保存时以空串覆盖数据库已有提示词。补全文字段后，前端可正确回填，不再洗掉数据。
...
// CustomStageResponse 自定义阶段响应（供前端流程搭建器使用）
//
// BugFix（自定义阶段提示词无法保存）：
//   原响应仅回 HasPrompt 布尔值，前端编辑已有阶段时拿不到提示词原文，
//   只能以空串塞进弹窗，保存时空串覆盖数据库已有提示词。
//   此处补齐 SystemPrompt / PromptVariants / OutputFormat 三个全文字段，
//   由 ListCustomStages / CreateCustomStage 从 workshop_stages 全量字段回填，
//   前端据此正确回填编辑弹窗，不再洗掉数据。
type CustomStageResponse struct {
        ...
        GateMode       string `json:"gate_mode"`       // 门控模式
        Skippable      bool   `json:"skippable"`       // 是否可跳过
        HasPrompt      bool   `json:"has_prompt"`      // 是否已配置提示词（保留，前端徽章用）
        SystemPrompt   string `json:"system_prompt"`   // BugFix：系统提示词全文（供编辑回填）
        PromptVariants string `json:"prompt_variants"` // BugFix：对话策略变体 JSON 全文（供编辑回填）
        OutputFormat   string `json:"output_format"`   // BugFix：产出物格式 JSON 全文（供编辑回填）
}
