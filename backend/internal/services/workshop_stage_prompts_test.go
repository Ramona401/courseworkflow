...
//
// BugFix（既有失败测试对齐当前实现）：
//   TestBuildLessonStructurePrompt_DesignStage 原断言 Contains(result,"概要") 检查一个
//   早期版本才有的"概要模式"字样。BuildLessonStructurePrompt 的 design 分支后续演进为
//   完整的板块+环节+时长渲染，输出引导语是"…教学设计须遵循…""按以上环节结构来设计…"，
//   源码中已无"概要"二字。此为测试断言陈旧，非实现缺陷（实现自洽合理、线上正用）。
//   故将该条断言改为检查 design 分支真实存在、且能与 write 分支"严格按照"区分开的标志串
//   「教学设计须遵循」，只改这一条断言，实现零改动。
...
func TestBuildLessonStructurePrompt_DesignStage(t *testing.T) {
        ...
        // 断言对齐当前实现：design 分支输出"教学设计须遵循"引导语（区别于 write 的"严格按照"），
        // 早期"概要模式"字样已随实现演进移除，源码中不再存在。
        if !strings.Contains(result, "教学设计须遵循") {
                t.Error("design阶段应使用教学设计引导模式（区别于write的严格模式）")
        }
