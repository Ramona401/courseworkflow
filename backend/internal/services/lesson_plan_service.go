...
//   ListLessonPlans 新增 callerID string 入参（放在最前），透传给
//   repository.ListLessonPlans。根因是 senior_operator 未绑学校管理员时其 scope
//   fail-closed 成空集，作者查自己教案被空白名单误伤，只剩名下 approved 那 1 条
//   命中"共享可见"分支。callerID 让 repo 层能加"作者就是当前登录用户自己时恒放行"
//   分支，保证任何用户始终能看到自己的全部教案，且不影响教案库页可见性。
...
func (s *LessonPlanService) ListLessonPlans(ctx context.Context, callerID string, authorID string, groupID string, status string, subject string, grade string, limit int, offset int, qualityLevel int, structureType int, cognitiveLevel int, pedagogyIntensity int, scope *DataScope) (*models.LessonPlanListResponse, error) {
	...
	items, total, err := repository.ListLessonPlans(ctx, callerID, authorID, groupID, status, subject, grade, limit, offset, qualityLevel, structureType, cognitiveLevel, pedagogyIntensity, scopeUserIDs, scopeIsAdmin)
	if err != nil {
		return nil, err
	}
	if items == nil {
...
func (s *LessonPlanService) PublishPersonal(ctx context.Context, id string, callerID string) error {
	...
	// v193放宽：允许「个人已发布」状态的教案再次发布（更新已发布版本）。
	// 背景：发布后状态变为 LPStatusPublishedPersonal，原校验只认 draft/revision，
	// 导致老师"发布→回来改→再发布"被拦失败。本函数仅改状态字段、无写记录/推送等副作用，
	// published_personal→published_personal 再发布幂等无害；状态机别处（如 SubmitForReview）
	// 本就接纳 published_personal 流转，放宽此处反而更一致。
	if lp.Status != models.LPStatusDraft &&
		lp.Status != models.LPStatusRevision &&
		lp.Status != models.LPStatusPublishedPersonal {
		return errors.New("只有草稿、退回或已发布状态的教案可以个人发布")
	}
	// v168：正文非空硬校验
