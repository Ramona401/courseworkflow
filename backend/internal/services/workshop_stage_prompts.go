...
//   - 非空字符串 → 整段替换第 4 层(阶段角色提示词+变体段),其他层保持不变
//
// 这样做的好处:
//  1. 保留第 1/2/3/3.5 层(配方/前序产出/组件/教案结构),不损失任何上下文
//  2. 保留第 5 层(对话规范),AI 仍知道如何输出
//  3. 仅替换"AI 扮演什么角色/用什么语气"这部分,由助手 full_prompt 完全决定
//  4. 符合规划:助手是"预组织好的 AI 指导方案",应整段注入而非拼接
func BuildStageSystemPromptV2(
	...
	// ==================== 第4层:阶段角色提示词(v110 新增助手注入) ====================
	// v110 关键改造:如果传入了 assistantPrompt,整段替换本层
	// - 助手 prompt 来自用户在评审工作台/工坊选中的 AI 助手 full_prompt
	// - 助手 prompt 已经包含完整角色定位+专业要求,不需再拼 stage.SystemPrompt
	// - 变体段(guided/peer/freeform)也不再附加,由助手 prompt 完全决定语气
	if strings.TrimSpace(assistantPrompt) != "" {
		sb.WriteString("\n")
		sb.WriteString(assistantPrompt)
		sb.WriteString("\n")
		wsPromptLog := logger.WithModule("workshop_stage_prompts")
		wsPromptLog.Info("使用 AI 助手 full_prompt 替换第4层阶段角色",
