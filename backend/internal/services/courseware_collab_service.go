...
func (s *CoursewareService) CanEditCourseware(ctx context.Context, cw *models.Courseware, userID string, role string) (bool, error) {
        if cw == nil {
                return false, fmt.Errorf("课件不存在")
        }
        // 1) 锁定态闸门：审核中（生产态或发布态任一锁定）一律禁改
        ...
        // 2) admin 直通（管理通道，独立于集体备课语义）
        if role == models.RoleAdmin {
                return true, nil
        }
        // 3) 作者 or 集体备课参与者 → 复用精简版判定（与 refine 三处完全同口径）
        return s.canRefineCourseware(ctx, cw, userID)
}

// canRefineCourseware 精简版微调权判定（不依赖 role，供 RefinePage/RegenerateSinglePage/RefineNav 三处统一调用）。
//
// 方案C 决策：集体备课的微调权只给"作者 + 被拉入的参与者"，admin 不在此特判
//   （admin 要改课件走自己的管理通道，不掺和教研活动的集体备课语义）。
//
// 判定（调用方已保证传入的 cw 非 nil；锁定态闸门由调用方在外层先卡，或此处再兜一道）：
//   - 锁定态（status=in_pipeline 或 publish_state=submitted）→ 任何人禁改，false
//   - 作者本人 → true
//   - collab_state=in_session 且 userID 在参与者名单 → true
//   - 其余 → false
func (s *CoursewareService) canRefineCourseware(ctx context.Context, cw *models.Courseware, userID string) (bool, error) {
        if cw == nil {
                return false, fmt.Errorf("课件不存在")
        }
        // 锁定态闸门（兜底再卡一道，保证本函数单独调用也安全）
        if cw.Status == models.CoursewareStatusInPipeline {
                return false, nil
        }
        if cw.PublishState == models.CWPublishSubmitted {
        ...
        if cw.CollabState == models.CWCollabInSession {
                isMember, err := repository.IsCollabMember(ctx, cw.ID, userID)
                if err != nil {
                        return false, err
                }
                if isMember {
...
func (s *CoursewareService) GetCollabStatus(ctx context.Context, coursewareID string, userID string, role string) (*models.CollabStatusResponse, error) {
        cw, err := repository.GetCoursewareByID(ctx, coursewareID)
        if err != nil {
        ...
        canEdit, err := s.CanEditCourseware(ctx, cw, userID, role)
        if err != nil {
                return nil, err
        }
