...
//
// 包含：阶段查询、进度管理、合并算法、阶段前进/跳过/回退、产出物保存、提示词上下文加载
//
// v76拆分：组件推荐+自定义阶段CRUD 移至 workshop_stage_components.go
// v77改动：ResetStage 改为按阶段分隔符截断对话（而非清空全部）
...
// v77b：持久化阶段分隔符到对话记录（供前端按阶段过滤+后端截断使用）
...
// ==================== 11. 重启指定阶段（v77改进：截断对话而非清空）====================

// ResetStage 重启指定阶段
//
// v77改进逻辑：
//   1. 验证教案所有权和阶段有效性
//   2. 重置目标阶段的产出物（structured_output/narrative 清空，status→in_progress）
//   3. 删除目标阶段之后所有阶段的产出物
//   4. 如果目标阶段是 write/revise，清空教案正文（ContentMarkdown）
//   5. 将 current_stage 设回目标阶段
//   6. 【v77改进】按阶段分隔符截断对话记录，保留目标阶段之前的历史对话
//      若找不到分隔符则清空全部（兜底）
//   7. 重新触发目标阶段的AI开场白
...
// v77改进：按阶段分隔符截断对话记录（保留目标阶段之前的历史）
// 构建阶段代码→阶段名映射表
