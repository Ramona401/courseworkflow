...
func NewLessonPlanGenService(
	cfg interface{ GetAESKey() string },
) *LessonPlanGenService {
...
func (s *LessonPlanGenService) SetAssistantService(
...
func (s *LessonPlanGenService) SetTextbookServiceForStage(
	ts *TextbookService,
) {
...
func (s *LessonPlanGenService) genStageOpeningMessage(
	...
	stageSystemPrompt, err :=
		s.stageService.LoadStagePromptContextV2(
			ctx,
			lp,
			snapshots[0].StageCode,
			assistantPrompt,
			"",
		)
	if err != nil {
	...
	userPrompt := BuildStageOpeningPrompt(
		lp,
		stage,
		snapshots[0].StageOrder,
		len(snapshots),
	)
...
func (s *LessonPlanGenService) Chat(
	...
	// 当前Chat已经成功取得lesson_plan_ai唯一任务所有权。
	// 后续同步整稿意图可能需要在真正启动AI前切换到write/revise；
	// 仅把本任务句柄写入私有context能力，不对外部HTTP阶段操作放宽AI busy硬闸。
	ctx = withLessonPlanAIStageMutationOwner(
		ctx,
		task,
		lp.ID,
	)
	...
	lp, err =
		s.ensureLegacyCompleteLessonPlanStageRuntimeForChat(
			ctx,
			lp,
		)
	if err != nil {
	...
	preparedPlan, err :=
		s.prepareLessonPlanWholeDraftChatIntent(
			ctx,
			lp,
			req,
			callerID,
	...
	// 自动切阶段完成后才读取Working Memory，避免把analyze/design旧轮次当成write当前轮。
	currentStageMsgs, err :=
	...
	userMsg :=
		buildLessonPlanChatUserMessage(
			req,
		)
	...
	assistantResolution :=
		s.resolveAssistantPromptForReceipt(
			ctx,
			lp,
			req.AssistantID,
			callerID,
	...
	assistantPrompt :=
		assistantResolution.Prompt
	assistantLabel := ""

	if assistantResolution.Receipt != nil {
		assistantLabel =
			assistantResolution.Receipt.Name
	}
...
func (s *LessonPlanGenService) resolveAssistantPrompt(
	...
	if assistantID != "" {
		...
		return assistant.FullPrompt,
			assistant.Name
	}

	prefID, found, prefErr :=
		repository.GetPref(
			ctx,
			callerID,
			lp.Subject,
