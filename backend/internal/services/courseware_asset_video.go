...
func (s *CoursewareAssetService) GenerateVideo(
	...
	//    两步流：前端把"已确认的首帧图URL"作为 RefImageURL 传入，实现图生视频，
	//    人物风格在首帧图这一关已锁定，视频只负责让静图动起来。
	refURL := ""
	if req.RefImageURL != "" {
	...
	// 6.1 首帧溯源：两步流传入了首帧图 asset_id 时，写 metadata 建立"视频←首帧图"血缘。
	//      用 encoding/json 安全序列化（不手拼字符串）；写失败仅 WARN，不阻断已提交的视频任务。
	if strings.TrimSpace(req.SourceFrameAssetID) != "" {
		metaBytes, mErr := json.Marshal(map[string]string{
			"source_frame_asset_id": strings.TrimSpace(req.SourceFrameAssetID),
		})
		if mErr != nil {
			cwAssetLog.Warn("视频首帧溯源JSON序列化失败(跳过溯源,不影响视频生成)",
				"asset_id", asset.ID,
				"source_frame_asset_id", req.SourceFrameAssetID,
				"error", mErr,
			)
		} else if updErr := repository.UpdateCWAssetMetadata(ctx, asset.ID, string(metaBytes)); updErr != nil {
			cwAssetLog.Warn("写入视频首帧溯源metadata失败(不影响视频生成)",
				"asset_id", asset.ID,
				"source_frame_asset_id", req.SourceFrameAssetID,
				"error", updErr,
			)
		} else {
			cwAssetLog.Info("视频已记录首帧溯源",
				"asset_id", asset.ID,
				"source_frame_asset_id", req.SourceFrameAssetID,
			)
		}
	...
	cwAssetLog.Info("视频生成任务已提交",
		...
		"has_source_frame", strings.TrimSpace(req.SourceFrameAssetID) != "",
	)
