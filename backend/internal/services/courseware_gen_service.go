...
//
// 教案原文校准改造（本次）：GeneratePreviewPages / GenerateRemainingPages 两处入口各调
//   loadLessonPlanContextForGen(ctx, cw) 一次性取教案正文（判 source_type=lesson_plan，非教案来源返空串），
//   随 buildPreviewUserPrompt / buildBatchUserPrompt 逐页传入。build 函数内部按页定向匹配教案相关片段
//   作为事实校准锚注入，令生成的课件忠实教案、不脑补。取数与匹配逻辑见 courseware_gen_lesson_context.go。
//   教案正文入口取一次、全页共用（并发 goroutine 只读，天然安全），不新增每页查库开销。
//
// 关联文件：
//   - courseware_gen_refine.go：导航栏微调+单页微调+单页重生
