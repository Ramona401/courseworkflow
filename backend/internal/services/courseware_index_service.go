...
//
// 对齐校验接线（课件↔教案对齐报告功能）：
//   - 结构体新增 alignmentService 字段 + SetAlignmentService 注入口（routes.go 注入）。
//   - saveAndBroadcast 末尾统一触发对齐校验（异步、非阻塞）：覆盖首次生成/AI改方案/
...
// extractLessonPlanContent 提取教案正文供 AI 使用。
// 行为与包级公共函数 ExtractLessonPlanContentForCW（courseware_lesson_extract.go）保持一致；
